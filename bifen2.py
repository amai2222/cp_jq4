...
class ScoreApp(tk.Tk):
    ...
    def fetch_all_data(self, lottery_type, issue):
        try:
            ...
            for i, match in enumerate(matches, 1):
                ...
                elif final_match_data.get('playItemList'):
                    hda_item = next((item for item in final_match_data['playItemList'] if item.get('playCode') in ['HDA', 'HHDA'] and lottery_type == 'jczq'), None)
                    if hda_item: let_ball = str(hda_item.get('concede', '0'))
                
                # 在这里调用_calculate_results
    ...
    # ======================= v37.2 核心修正：修复_calculate_results函数逻辑错误 =========================
