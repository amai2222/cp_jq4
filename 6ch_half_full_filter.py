...
"""
6场半全场过滤器
数据格式：12位数字，每2位代表1场比赛（半场结果+全场结果）
半场/全场结果：3（胜）、1（平）、0（负）
"""
...
class SixCHalfFullFilter:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("6场半全场过滤器")
    ...
    def _init_filter_vars(self):
        ...
        for i in range(6):
            game_vars = {}
            for result in ['33', '31', '30', '13', '11', '10', '03', '01', '00']:
                game_vars[result] = tk.BooleanVar()
            self.half_full_vars.append(game_vars)
    ...
    def _create_data_acquisition_ui(self, parent):
        ...
        ttk.Label(data_acq_frame, text="6场半全场数据获取", font=('Microsoft YaHei UI', 12, 'bold')).pack(anchor=tk.W)
    ...
    def _create_match_table(self, parent):
        ...
        # 表头
        headers = ["场次", "联赛", "开赛日期", "主队 VS 客队", "半全场选择"]
    ...
    def _create_match_row(self, parent, game_idx, row):
        ...
        # 半全场选项：33, 31, 30, 13, 11, 10, 03, 01, 00
        half_full_options = ['33', '31', '30', '13', '11', '10', '03', '01', '00']
        for j, option in enumerate(half_full_options):
            var = tk.BooleanVar()
    ...
    def _create_input_ui(self, parent):
        """创建输入界面"""
        # 输入说明
        ttk.Label(parent, text="请输入6场半全场投注结果，每行一个投注，格式如：3311031100", 
    ...
    def _create_filter_ui(self, parent):
        """创建过滤器界面"""
        # 半全场过滤器
        filter_frame = ttk.LabelFrame(parent, text="半全场过滤器", padding="10")
        filter_frame.pack(fill=tk.X, pady=(0, 10))
        
        # 创建6场比赛的半全场过滤
        for i in range(6):
            game_frame = ttk.LabelFrame(filter_frame, text=f"第{i+1}场", padding="5")
            game_frame.pack(fill=tk.X, pady=(0, 5))
            
            # 半全场选项
            half_full_options = ['33', '31', '30', '13', '11', '10', '03', '01', '00']
            for j, option in enumerate(half_full_options):
                cb = tk.Checkbutton(game_frame, text=option, 
                                  variable=self.half_full_vars[i][option],
                                  font=('Microsoft YaHei UI', 8))
    ...
    def _check_bet_format(self, bet):
        ...
        for i in range(0, 12, 2):
            half_full = bet[i:i+2]
            if half_full not in ['33', '31', '30', '13', '11', '10', '03', '01', '00']:
                return False
    ...
    def apply_filter(self):
        ...
        for bet in self.betting_data:
            if self._check_half_full_filter(bet):
    ...
    def _check_half_full_filter(self, bet):
        """检查半全场过滤条件"""
        # 解析投注数据（12位数字，每2位代表一场比赛的半全场结果）
        for i in range(6):
            half_full_result = bet[i*2:(i+1)*2]
            
            # 检查是否有任何半全场选项被选中
            any_selected = any(self.half_full_vars[i][result].get() for result in 
                             ['33', '31', '30', '13', '11', '10', '03', '01', '00'])
            
            if any_selected:  # 如果有选项被选中
                if not self.half_full_vars[i][half_full_result].get():
                    return False
    ...
    def export_result(self):
        ...
        if self.filtered_data:
            # 按升序排列
            sorted_data = sorted(self.filtered_data)
            filename = f"6场半全场过滤结果_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            ...
            # 如果没有过滤结果，导出投注数据
            sorted_data = sorted(self.betting_data)
            filename = f"6场半全场投注数据_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f"6场半全场投注数据\n")
                f.write(f"导出时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    ...
    def _display_match_details(self, match_info):
        ...
        try:
            ...
            for match in match_list[:6]:  # 只显示前6场
                # 正确解析API数据
    ...
    def _update_match_table(self, match_num, master_team, guest_team, league, match_time):
        """更新对阵表格显示"""
        try:
            # 将match_num转换为索引（假设match_num是1-6）
            ...
            # 确保索引在有效范围内
            if 0 <= game_idx < 6 and game_idx in self.match_labels:
    ...
    def show_frequency_filter(self):
        ...
        for bet in data_source:
            for i in range(6):
                half_full = bet[i*2:(i+1)*2]
                key = f"第{i+1}场_{half_full}"
                freq_stats[key] = freq_stats.get(key, 0) + 1
        
        # 创建频率调整窗口
        freq_window = tk.Toplevel(self.root)
        freq_window.title("6场半全场频率缩水")
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="6场半全场频率缩水",
        ...
        for i in range(6):
            ...
            for j, option in enumerate(half_full_options):
                ...
                option_frame.grid(row=0, column=j, padx=5, pady=5)
                
                ttk.Label(option_frame, text=option, font=('Microsoft YaHei UI', 9)).pack()
                ttk.Label(option_frame, text=f"当前:{current_freq}", font=('Microsoft YaHei UI', 8)).pack()
                
                var = tk.StringVar(value=str(current_freq))
                freq_vars[key] = var
                
                entry = ttk.Entry(option_frame, textvariable=var, width=8)
                entry.pack()
        ...
        try:
            ...
            for bet in data_source:
                ...
                for i in range(6):
                    ...
                    # 计算当前该比分的频率
                    current_count = sum(1 for b in filtered_data if b[i*2:(i+1)*2] == half_full)
