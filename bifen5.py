...
class ScoreApp(tk.Tk):
    def __init__(self):
        super().__init__()
        # 版本号
        self.title("实时比分查看工具 (v55.0 - 完整修复版)")
    ...
    def create_tab(self, parent, name, lottery_type):
        ...
        columns = ('match_num', 'match_time', 'sequence', 'league', 'home_team', 'vs', 'away_team', 'half_score', 'status', 'result', 'home_sp', 'draw_sp', 'away_sp', 'score', 'path')
        ...
        headings = {'match_num': '场次', 'match_time': '时间', 'sequence': '次序', 'league': '赛事', 'home_team': '主队', 'vs': '', 'away_team': '客队', 'half_score': '半场', 'status': '状态', 'result': '赛果', 'home_sp': '胜', 'draw_sp': '平', 'away_sp': '负', 'score': '比分', 'path': '正反路'}
        widths = {'match_num': 80, 'match_time': 110, 'sequence': 50, 'league': 90, 'home_team': 150, 'vs': 30, 'away_team': 150, 'half_score': 80, 'status': 80, 'result': 120, 'home_sp': 60, 'draw_sp': 60, 'away_sp': 60, 'score': 100, 'path': 60}
        anchors = {'match_num': tk.CENTER, 'match_time': tk.CENTER, 'sequence': tk.CENTER, 'league': tk.CENTER, 'home_team': tk.E, 'vs': tk.CENTER, 'away_team': tk.W, 'half_score': tk.CENTER, 'status': tk.CENTER, 'result': tk.CENTER, 'home_sp': tk.CENTER, 'draw_sp': tk.CENTER, 'away_sp': tk.CENTER, 'score': tk.CENTER, 'path': tk.CENTER}
        for col in columns: tree.heading(col, text=headings[col]); tree.column(col, width=widths[col], anchor=anchors[col])
        tree.tag_configure('finished', foreground='black'); tree.tag_configure('in_progress', foreground='blue'); tree.tag_configure('not_started', foreground='gray'); tree.tag_configure('cancelled', foreground='purple')
    ...
    def fetch_all_data(self, lottery_type, issue):
        try:
            ...
            for i, match in enumerate(matches, 1):
                ...
                let_ball = '0'
                play_map = final_match_data.get('playMap')
                if play_map:
                    let_node = None
                    if lottery_type == 'bjdc':
                        let_node = play_map.get('BJ_HDA')
                    else:
                        let_node = play_map.get('HHDA')
                    if let_node:
                        let_ball = str(let_node.get('concede', '0'))
                hda_odds = self._get_hda_odds(final_match_data, lottery_type)
                home_sp, draw_sp, away_sp = hda_odds.get('主胜', '-'), hda_odds.get('平', '-'), hda_odds.get('客胜', '-')
                ...
                display_handicap = ""
                if let_ball and let_ball != '0' and let_ball != '0.0':
                    try: handicap_val = float(let_ball); display_handicap = f"({handicap_val:+g})"
                    except (ValueError, TypeError): pass
                home_team_display = f"{home_team} {display_handicap}".strip()
                row_values = (match_num, match_time_display, str(match.get('sort', i)), league_name, home_team_display, 'vs', away_team, half_time_str, status_str, result_str, home_sp, draw_sp, away_sp, score_str, path_str)
                tags = (status_category,)
                processed_data.append({'values': row_values, 'tags': tags, 'full_data': final_match_data, 'status_category': status_category})
    ...
    def show_odds_popup(self, match_data):
        ...
        hda_node = play_map.get('HDA')
        if hda_node:
            frame = ttk.LabelFrame(content_frame, text="胜平负 (不让球)", padding=10); frame.pack(fill=tk.X, pady=5)
            ods, _ = self._parse_odds_from_node(hda_node, {})
            for name in ["主胜", "平", "客胜"]:
                ttk.Label(frame, text=f"{name}: {ods.get(name, 'N/A')}", width=15).pack(side=tk.LEFT, padx=5)
        hhda_node = play_map.get('HHDA') or play_map.get('BJ_HDA')
        if hhda_node:
            concede = hhda_node.get('concede', 0)
            try:
                concede_num = float(concede)
                frame_text = f"让球胜平负 ({concede_num:+g})"
            except (ValueError, TypeError):
                frame_text = f"让球胜平负 ({concede})"
            frame = ttk.LabelFrame(content_frame, text=frame_text, padding=10); frame.pack(fill=tk.X, pady=5)
            ods, _ = self._parse_odds_from_node(hhda_node, {})
            name_map = {"主胜": "让胜", "平": "让平", "客胜": "让负"}
            for orig_name, display_name in name_map.items():
                ttk.Label(frame, text=f"{display_name}: {ods.get(orig_name, 'N/A')}", width=15).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(button_frame, text="调试赔率", command=lambda: self.show_odds_debug_info(match_data)).pack(side=tk.LEFT, padx=(0,5))
    ...
    def show_odds_debug_info(self, match_data):
        current_tab_info = self.get_current_tab_info()
        lottery_type = current_tab_info['type'] if current_tab_info else ''
        debug_text = self._generate_debug_string(match_data, lottery_type)
        messagebox.showinfo("赔率调试报告", debug_text)
        
    def _generate_debug_string(self, match_data, lottery_type):
        report = []
        report.append("--- 开始调试赔率获取流程 ---")
        play_map = match_data.get('playMap')
        if not play_map:
            report.append("错误: 顶级数据中未找到 'playMap' 节点。无法获取任何玩法赔率。")
            return "\n".join(report)
        report.append("1. 'playMap' 节点存在。")
        target_node_name = ''
        if lottery_type == 'jczq':
            target_node_name = 'HDA'
            report.append(f"2. 当前为“竞彩”，目标是查找不让球赔率节点: '{target_node_name}'。")
        elif lottery_type == 'bjdc':
            target_node_name = 'BJ_HDA'
            report.append(f"2. 当前为“北单”，目标是查找赔率节点: '{target_node_name}'。")
        else:
            report.append(f"2. 当前彩种 ('{lottery_type}') 不需要在主列表显示赔率。"); return "\n".join(report)
        node = play_map.get(target_node_name)
        if not node:
            report.append(f"3. 失败: 在 'playMap' 中未找到目标节点 '{target_node_name}'。"); report.append("\n--- 结论: 无法获取赔率，因为缺少关键数据节点。 ---"); return "\n".join(report)
        report.append(f"3. 成功: 找到了目标节点 '{target_node_name}'。")
        _, debug_msg = self._parse_odds_from_node(node, {})
        report.append(debug_msg); return "\n\n".join(report)
        
    def _get_hda_odds(self, match_data, lottery_type):
        play_map = match_data.get('playMap', {})
        if lottery_type == 'jczq': node = play_map.get('HDA')
        elif lottery_type == 'bjdc': node = play_map.get('BJ_HDA')
        else: return {}
        odds_dict, _ = self._parse_odds_from_node(node, {})
        return odds_dict
        
    def _parse_odds_from_node(self, node, _):
        debug_report = []
        if not node:
            debug_report.append(f"4. 失败: 传入的节点本身为空。")
            debug_report.append("\n--- 结论: 无法解析，因为数据节点不存在。 ---")
            return {}, "\n".join(debug_report)
        odds_list = node.get('playItemList') or node.get('options')
        if not odds_list:
            debug_report.append(f"4. 失败: 在节点内, 未找到 'playItemList' 或 'options' 列表。")
            debug_report.append(f"   > 节点包含的键有: {list(node.keys())}")
            debug_report.append("\n--- 结论: 无法获取赔率，因为赔率列表不存在。 ---")
            return {}, "\n".join(debug_report)
        debug_report.append(f"4. 成功: 找到了赔率列表 (共 {len(odds_list)} 项)。")
        debug_report.append("\n5. 开始逐项解析赔率列表...")
        temp_dict = {}
        name_map_new = {"Home": "主胜", "Draw": "平", "Away": "客胜"}
        for i, item in enumerate(odds_list):
            item_report_lines = [f"  - 第 {i+1} 项: {item}"]
            name = name_map_new.get(item.get('name')) or item.get('playItemName')
            sp = item.get('sp') or item.get('odds')
            if name and name in ["主胜", "平", "客胜"]:
                if sp:
                    temp_dict[name] = sp
                    item_report_lines.append(f"    - 成功: 解析到 '{name}' -> '{sp}'")
                else: item_report_lines.append(f"    - 失败: 找到名称 '{name}' 但赔率(sp/odds)为空。")
            else: item_report_lines.append(f"    - 忽略: 名称 '{name}' 不是目标(主胜/平/客胜)。")
            debug_report.append("\n".join(item_report_lines))
        debug_report.append("\n6. 解析完成。")
        if len(temp_dict) == 3:
            debug_report.append(f"7. 成功: 已集齐胜平负3项赔率: {temp_dict}")
            debug_report.append("\n--- 结论: 赔率获取成功！ ---")
        else:
            debug_report.append(f"7. 失败: 未能集齐3项赔率，只找到了 {len(temp_dict)} 项。")
            debug_report.append(f"   > 已找到的赔率: {temp_dict if temp_dict else '无'}")
            debug_report.append("\n--- 结论: 无法获取完整的胜平负赔率。 ---")
        return temp_dict, "\n".join(debug_report)
    ...
    def _calculate_results(self, home_score, guest_score, let_ball, status_category, lottery_type, home_half_score=None, guest_half_score=None):
        ...
        try:
            ...
            if let_f != 0.0:
                rqspf_map = {1: "让胜", 0: "让平", -1: "让负"}; rq_comp = 1 if (home_s + let_f) > guest_s else -1 if (home_s + let_f) < guest_s else 0; rq_spf = rqspf_map.get(rq_comp)
                if lottery_type == 'bjdc': return rq_spf
                return f"{spf_text} / {rq_spf}"
        except (ValueError, TypeError): pass
        return spf_text
    ...
    def _calculate_path(self, home_score, guest_score, status_category, odds_dict):
        ...
        if not odds_dict or len(odds_dict) < 3: return ""
        float_odds = {};
        for name, sp in odds_dict.items():
            try: float_odds[name] = float(sp)
            except (ValueError, TypeError): continue
        if len(float_odds) < 3: return ""
    ...
    def _get_basic_info(self, match_data, lottery_type, index, issue):
        if lottery_type == 'bjdc': match_num = str(match_data.get('jcNum')) if match_data.get('jcNum') else "无北单号"
    ...
    def update_ui_with_results(self, tree, data, original_issue):
        ...
        if isinstance(data, str): 
            ...
            if lottery_type in ['jqs', 'bqc', 'sfc']:
                final_result_string = "".join([str(row['values'][9]) if row.get('status_category') == 'finished' else '*' for row in data]) # Column index may need adjustment, it's 'result'
                # Find index of 'result'
                columns = ('match_num', 'match_time', 'sequence', 'league', 'home_team', 'vs', 'away_team', 'half_score', 'status', 'result', 'home_sp', 'draw_sp', 'away_sp', 'score', 'path')
                result_col_index = columns.index('result')
                final_result_string = "".join([str(row['values'][result_col_index]) if row.get('status_category') == 'finished' else '*' for row in data])
    ...
    def show_debug_window(self, data, parent_popup):
        ...
        try:
            ...
            text_widget.insert(tk.END, f"无法格式化JSON数据: {e}\n\n原始数据:\n{data}")
