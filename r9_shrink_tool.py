...
class R9ShrinkTool:
    def __init__(self, root):
        ...
        self.banker_vars = [tk.IntVar(value=0) for _ in range(14)]  # 胆码选择
        self.draw_id_var = tk.StringVar()
        self.status_var = tk.StringVar(value="欢迎使用任九缩水工具")
    ...
    def _create_widgets(self):
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="任九专业缩水工具", style='Title.TLabel')
    ...
    def _create_betting_area(self, parent):
        ...
        for i in range(14):
            ...
            # 胆码选择
            banker_cb = ttk.Checkbutton(table_frame, variable=self.banker_vars[i])
            banker_cb.grid(row=i+1, column=6, padx=3, pady=3)
    ...
    def refresh_draw_list(self):
        ...
        def worker():
            ...
            result = self.api.get_draw_list('r9', 10)
    ...
    def fetch_draw_details(self):
        ...
        def worker():
            ...
            result = self.api.get_draw_details('r9', draw_id)
    ...
    def calculate_bets(self):
        ...
        for i in range(14):
            ...
            if self.banker_vars[i].get():
                banker_indices.add(i)
        
        # 验证胆码
        if len(banker_indices) > 9:
            UIHelper.show_error(self, "错误", "胆码不能超过9个")
            return
        
        # 计算需要选择的场次数
        total_selected = sum(1 for opts in selected_matches if opts)
        if total_selected < 9:
            UIHelper.show_error(self, "错误", f"至少需要选择9场比赛，当前选择了{total_selected}场")
            return
        
        # 生成任九组合
        self.original_bets = self._generate_r9_combinations(selected_matches, banker_indices)
    ...
    def _generate_r9_combinations(self, selected_matches, banker_indices):
        """生成任九组合"""
        bets = []
        
        # 获取非胆码场次
        non_banker_indices = [i for i in range(14) if i not in banker_indices and selected_matches[i]]
        ...
        # 生成所有可能的9场组合
        for dynamic_combo in itertools.combinations(non_banker_indices, num_to_choose):
            r9_indices = tuple(sorted(list(banker_indices) + list(dynamic_combo)))
            r9_options = [selected_matches[idx] for idx in r9_indices]
            
            # 生成该组合的所有投注
            for r9_outcomes in itertools.product(*r9_options):
                bet_list = ['*'] * 14
                for match_idx, outcome in zip(r9_indices, r9_outcomes):
                    bet_list[match_idx] = outcome
                bets.append("".join(bet_list))
    ...
    def start_shrink(self):
        ...
        def worker():
            ...
            # 应用过滤
            self.filtered_bets = self._apply_r9_filters(self.original_bets, filters)
    ...
    def _apply_r9_filters(self, bets, filters):
        """应用任九专用过滤条件"""
        filtered = []
        
        for bet in bets:
            # 只考虑非*的场次
            non_star_bet = ''.join(c for c in bet if c != '*')
            
            # 统计胜平负
            wins = non_star_bet.count('3')
            draws = non_star_bet.count('1')
            loses = non_star_bet.count('0')
            
            # 计算积分和
            points = wins * 3 + draws * 1
            
            # 计算断点数
            breaks = sum(1 for i in range(len(non_star_bet) - 1) if non_star_bet[i] != non_star_bet[i+1])
            
            # 计算连续场次
            consecutive_wins = self._get_max_consecutive(non_star_bet, '3')
            consecutive_draws = self._get_max_consecutive(non_star_bet, '1')
            consecutive_loses = self._get_max_consecutive(non_star_bet, '0')
    ...
    def start_wheel(self):
        ...
        def worker():
            ...
            self.wheeled_bets = WheelMatrix.wheel_guarantee_8(source_bets, 'r9')
