...
class FourteenCHFilter:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("十四场过滤器")
    ...
    def _init_filter_vars(self):
        ...
        for i in range(14):
            ...
            self.fourteen_vars.append(position_vars)
    ...
    def _create_data_acquisition_ui(self, parent):
        ...
        ttk.Label(data_acq_frame, text="十四场数据获取", font=('Microsoft YaHei UI', 12, 'bold')).pack(anchor=tk.W)
    ...
    def _create_match_row(self, parent, game_idx, row):
        ...
        # 投注选项：3（胜）、1（平）、0（负）
        fourteen_options = ['3', '1', '0']
        for j, option in enumerate(fourteen_options):
            var = tk.BooleanVar()
    ...
    def _create_input_ui(self, parent):
        """创建输入界面"""
        # 输入说明
        ttk.Label(parent, text="请输入十四场投注结果，每行一个投注，格式如：31031031031031", 
    ...
    def _create_filter_ui(self, parent):
        """创建过滤器界面"""
        # 十四场过滤器
        filter_frame = ttk.LabelFrame(parent, text="十四场过滤器", padding="10")
        ...
        for i in range(14):
            ...
            for j, option in enumerate(fourteen_options):
                cb = tk.Checkbutton(position_frame, text=option, 
                                  variable=self.fourteen_vars[i][option],
                                  font=('Microsoft YaHei UI', 8))
    ...
    def apply_filter(self):
        ...
        for bet in self.betting_data:
            if self._check_fourteen_filter(bet):
    ...
    def _check_fourteen_filter(self, bet):
        """检查十四场过滤条件"""
        # 解析投注数据（14个位置）
        for i in range(14):
            ...
            # 检查是否有任何投注选项被选中
            any_selected = any(self.fourteen_vars[i][result].get() for result in ['3', '1', '0'])
            
            if any_selected:  # 如果有选项被选中
                if not self.fourteen_vars[i][position_result].get():
                    return False
    ...
    def export_result(self):
        ...
        if self.filtered_data:
            # 按升序排列
            sorted_data = sorted(self.filtered_data)
            filename = f"十四场过滤结果_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            ...
            # 如果没有过滤结果，导出投注数据
            sorted_data = sorted(self.betting_data)
            filename = f"十四场投注数据_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f"十四场投注数据\n")
                f.write(f"导出时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    ...
    def show_frequency_filter(self):
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="十四场频率缩水",
