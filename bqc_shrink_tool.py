...
class BQCShrinkTool:
    def __init__(self, root):
        self.root = root
        self.root.title("半全场专业缩水工具 V1.0")
        ...
        # UI变量
        self.match_vars = [[tk.IntVar(value=0) for _ in range(9)] for _ in range(6)]  # 6场，每场9个选项(33,31,30,13,11,10,03,01,00)
        self.draw_id_var = tk.StringVar()
        self.status_var = tk.StringVar(value="欢迎使用半全场缩水工具")
    ...
    def _create_widgets(self):
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="半全场专业缩水工具", style='Title.TLabel')
    ...
    def _create_betting_area(self, parent):
        ...
        # 表头
        headers = ["场次", "主队", "客队", "33", "31", "30", "13", "11", "10", "03", "01", "00"]
        ...
        for i in range(6):
            # 场次号
            ttk.Label(table_frame, text=f"第{i+1}场").grid(row=i+1, column=0, padx=3, pady=5)
            
            # 队伍名称（占位）
            ttk.Label(table_frame, text="主队", width=12).grid(row=i+1, column=1, padx=3, pady=5)
            ttk.Label(table_frame, text="客队", width=12).grid(row=i+1, column=2, padx=3, pady=5)
            
            # 投注选项
            for j in range(9):
                cb = ttk.Checkbutton(table_frame, variable=self.match_vars[i][j])
                cb.grid(row=i+1, column=j+3, padx=3, pady=5)
    ...
    def refresh_draw_list(self):
        ...
        def worker():
            ...
            result = self.api.get_draw_list('bqc', 10)
    ...
    def fetch_draw_details(self):
        ...
        def worker():
            ...
            result = self.api.get_draw_details('bqc', draw_id)
    ...
    def _update_match_info(self, data):
        ...
        if matches:
            ...
            for i, match in enumerate(matches[:6]):  # 只显示前6场
        ...
        for i in range(6):
            ...
            for j in range(9):
                if self.match_vars[i][j].get():
                    # 将索引转换为半全场结果
                    bqc_options = ['33', '31', '30', '13', '11', '10', '03', '01', '00']
                    options.append(bqc_options[j])
            if not options:
    ...
    def start_shrink(self):
        ...
        def worker():
            ...
            filters = {}
            try:
                if self.filter_vars['min_wins'].get():
                    filters['min_wins'] = int(self.filter_vars['min_wins'].get())
                if self.filter_vars['max_wins'].get():
                    filters['max_wins'] = int(self.filter_vars['max_wins'].get())
                if self.filter_vars['min_draws'].get():
                    filters['min_draws'] = int(self.filter_vars['min_draws'].get())
                if self.filter_vars['max_draws'].get():
                    filters['max_draws'] = int(self.filter_vars['max_draws'].get())
                if self.filter_vars['min_loses'].get():
                    filters['min_loses'] = int(self.filter_vars['min_loses'].get())
                if self.filter_vars['max_loses'].get():
                    filters['max_loses'] = int(self.filter_vars['max_loses'].get())
                if self.filter_vars['min_points'].get():
                    filters['min_points'] = int(self.filter_vars['min_points'].get())
                if self.filter_vars['max_points'].get():
                    filters['max_points'] = int(self.filter_vars['max_points'].get())
                if self.filter_vars['min_breaks'].get():
                    filters['min_breaks'] = int(self.filter_vars['min_breaks'].get())
                if self.filter_vars['max_breaks'].get():
                    filters['max_breaks'] = int(self.filter_vars['max_breaks'].get())
            except ValueError:
                def show_error():
                    UIHelper.hide_progress(self)
            ...
            # 应用过滤
            self.filtered_bets = self._apply_bqc_filters(self.original_bets, filters)
    ...
    def _apply_bqc_filters(self, bets, filters):
        """应用半全场专用过滤条件"""
        filtered = []
        ...
        for bet in bets:
            ...
            for i in range(0, len(bet), 2):
                result = bet[i:i+2]
                if result in ['33', '31', '30']:
                    wins += 1
                elif result in ['13', '11', '10']:
                    draws += 1
                elif result in ['03', '01', '00']:
                    loses += 1
            ...
            # 计算断点数
            breaks = 0
            for i in range(0, len(bet) - 2, 2):
                if bet[i:i+2] != bet[i+2:i+4]:
                    breaks += 1
    ...
    def start_wheel(self):
        ...
        def worker():
            ...
            self.wheeled_bets = WheelMatrix.wheel_guarantee_8(source_bets, 'bqc')
